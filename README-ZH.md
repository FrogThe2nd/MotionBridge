...
- L0/L1/L2/R0/R1/R2、双手/双脚参考、增益（围绕所选中心增减输出行程）、中心、死区、曲线、范围、反向与失联回中逻辑；每个轴都可独立启用偏好行程优化，在稳定往返后只补足短行程，并为额外动作保留余量。平移轴自动倍率最高 4×，旋转轴采用更保守的 2× 上限。
- USB 串口、Wi-Fi UDP（默认 `tcode.local:8000`）与 Intiface Desktop；初始状态始终未解锁输出。
- 三种输出共用独立的固定频率时钟，并按真实间隔生成 TCode；高级设置支持软启动、跨轴智能限制、逐轴速度限制、输出开关和安全位置。智能限制可选择驱动轴，并通过两个自由控制点动态调整当前轴保留的行程或响应速度；轴卡数值与 3D 预览始终显示最终处理信号，不受硬件输出是否解锁影响。
