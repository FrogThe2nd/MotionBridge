...
DeviceRouter::DeviceRouter(QObject* parent) : QObject(parent) {
    ...
    handy_ = new HandyPositionOutput(this);
    ...
    connect(handy_, &HandyPositionOutput::status_changed, this, [this](const QString& status, const bool connected) {
        if (mode_ != Mode::Handy) return;
        armed_ = handy_->armed();
        emit status_changed(status, connected);
    });
}
...
void DeviceRouter::send_output(const Axes& axes, const std::chrono::milliseconds interval, const bool force_full) {
    ...
    if (mode_ == Mode::Handy) handy_->send_position(axes[0], interval);
}
...
void DeviceRouter::emergency_stop() {
    ...
    send_intiface_zero();
    if (mode_ == Mode::Handy) handy_->emergency_stop();
    armed_ = false;
    reset_output_tracking();
